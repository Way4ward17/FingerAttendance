...
//            pst.setBinaryStream(10,datosHuella,(template.serialize().length));
...
//                    jLabel2.setIcon(newImage);
    ...
    public void preencherTabelaAttendance(String SQL, JTable tabela) {
        conecta.conexao();
        ArrayList dados = new ArrayList();
        String[] Colunas = new String[]{"ID", "","Date", "Matric Number", "Student ID"};
        conecta.executaSQL(SQL);
        try {
            conecta.rs.first();
            do {
                   System.out.println(conecta.rs.getString("id")+conecta.rs.getString("date")+conecta.rs.getString("matricnumber")+conecta.rs.getString("id_num"));
          
                dados.add(new Object[]{
                   
                    conecta.rs.getString("id"),
                    "",
                    conecta.rs.getString("date"),
                    conecta.rs.getString("matricnumber"),
                     conecta.rs.getString("id_num"),
                });
            } while (conecta.rs.next());
...
//                lbl.setIcon(i);
          ...
          byte[] img = (byte[]) table.getValueAt(row, 0);
                    ...
                    ImageIcon image = new ImageIcon(img);
                    Image im = image.getImage();
                    Image myImg = im.getScaledInstance(lbl.getWidth()+50, lbl.getHeight()+50,Image.SCALE_SMOOTH);
                    ImageIcon newImage = new ImageIcon(myImg);
